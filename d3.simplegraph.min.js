...
/* harmony default export */ var sg_grid_legend = (function (SimpleGraph) {
  ...
  SimpleGraph.prototype.drawLegend = function (position, anchor, bgstyle, itemsPerColumn, rowHeight, exclude) {
    ...
    var legendBg = legend.append("rect").attr("class", "sg-legend-bg").attr("x", 0).attr("y", 0);
    ...
    var columnNumber = 0;
    var columnItemCount = 0; // running position for next item

    var yOffset = bgstyle['padding-top'];
    var xOffset = bgstyle['padding-left']; // local function checks for new column and adjusts position if so
...
/* harmony default export */ var sg_data_point = (function (SimpleGraph) {
  ...
  SimpleGraph.prototype.addPointData = function (name, xValue, yValue, options) {
    ...
    if (options.shape) {
      if (typeof options.shape == "string" && ~sg_point_shapes.indexOf(options.shape)) {
  ...
  SimpleGraph.prototype.addPointsData = function (data, dataPointName, xValueName, yValueName, options) {
    ...
    options.y2Axis = !!options.y2Axis;
    options.showNulls = !!options.showNulls;
    options.additionalDataKeys = options.additionalDataKeys || null; // validate shape, if object type, map to local dict, otherwise save for loop as we get series names

    if (options.shape) {
      if (typeof options.shape !== "string" || !~sg_point_shapes.indexOf(options.shape)) {
        for (var series in options.shape) {
          if (~sg_point_shapes.indexOf(options.shape[series])) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var seriesName = !dataPointName ? i : data[i][dataPointName] ? data[i][dataPointName] : dataPointName,
          ...
          yValue = data[i][yValueName]; // add shape if provided as constant string

      if (options.shape) this.ptSeriesShapes[seriesName] = options.shape; // if any null x-values, skip

      if (xValue === undefined || xValue === null) continue; // nicely organize data
  ...
  SimpleGraph.prototype.setPointSeriesShape = function (name, shape) {
    ...
    if (typeof shape == "string" && ~sg_point_shapes.indexOf(shape)) {
...
/* harmony default export */ var sg_data_line = (function (SimpleGraph, d3) {
  ...
  SimpleGraph.prototype.addLinesDataFromPoints = function (style, interpolation, handleOverlap) {
    ...
    this.pointLines = []; // start looping

    for (var series in pointsBySeries) {
      var checkPoints = pointsBySeries[series].points;
      var yAxis = pointsBySeries[series].y2 ? this.y2 : this.y; // less than 2 points, skip
      ...
      for (var i = 0; i < checkPoints.length; i++) {
        var pointToAdd = checkPoints[i]; // always add point to line, will now do check beyond graph at drawing stage
        ...
        for (var j = 0; j < lineCoords.length; j++) {
          if (lineCoords[j][0] === pointToAdd.x) {
            exists = true; // if it exists, add to count and..
...
// CONCATENATED MODULE: ./src/sg.draw.js

/* harmony default export */ var sg_draw = (function (SimpleGraph, d3) {
  /**
  ...
  SimpleGraph.prototype.drawPoints = function () {
    this.removePoints();
    if (!this.points || this.points.length === 0) return this; // for 'this' references

    var self = this,
        color = this.color; // if necessary, remove points that extend beyond graph
    ...
    for (var series in pointsDataBySeries) {
      var shape = this.ptSeriesShapes[series],
  ...
  SimpleGraph.prototype._drawSquareDiamond = function (shape, data) {
    var self = this;
    this.svgGraph.selectAll(".sg-point.sg-point-sd").data(data).enter().append("rect").attr("series", function (d) {
      return d.series;
      ...
      return shape == "square" ? "" : "rotate(45,".concat(self.x.scale(d.x), ",").concat((d.y2 ? self.y2.scale : self.y.scale)(d.y), ")");
  ...
  SimpleGraph.prototype._drawCircle = function (shape, data) {
    var self = this;
    this.svgGraph.selectAll(".sg-point.sg-point-cr").data(data).enter().append("circle").attr("series", function (d) {
      return d.series;
    }).attr("class", "sg-point sg-point-cr").attr("r", function (d) {
      return 0.5 * (typeof d.size === "function" ? d.size() : d.size);
    }).attr("cx", function (d) {
      return self.x.scale(d.x);
    }).attr("cy", function (d) {
      return (d.y2 ? self.y2.scale : self.y.scale)(d.y);
    }).style("fill", function (d) {
      return d.wasNull ? "none" : self.getColorBySeriesName(d.series, true);
    }).style("stroke", function (d) {
      return !d.wasNull ? null : self.getColorBySeriesName(d.series, true);
  ...
  SimpleGraph.prototype._drawTriangleUp = function (shape, data) {
    var self = this;
    this.svgGraph.selectAll(".sg-point.sg-point-tu").data(data).enter().append("polygon").attr("series", function (d) {
      return d.series;
    }).attr("class", "sg-point sg-point-tu").attr("points", function (d) {
  ...
  SimpleGraph.prototype._drawTriangleDown = function (shape, data) {
    var self = this;
    this.svgGraph.selectAll(".sg-point.sg-point-td").data(data).enter().append("polygon").attr("series", function (d) {
      return d.series;
    }).attr("class", "sg-point sg-point-td").attr("points", function (d) {
      var size = size = typeof d.size === "function" ? d.size() : d.size,
          length = size * 1.519676,
          // side length of equilateral trangle of same area of square
      height = length * 0.86602,
  ...
  SimpleGraph.prototype.drawLines = function () {
    ...
    function addLine(lineData, lineCoords, className) {
      ...
      var addedLine = svgGraph.selectAll(".sg-temporary-line").data([lineCoords]).enter().append("path").attr("series", lineData.series).attr("class", className).style("fill", 'none').attr("d", d3.line().x(function (c) {
...
/* harmony default export */ var sg_tooltip = (function (SimpleGraph, d3) {
  ...
  SimpleGraph.prototype.addTooltipToPoints = function (tooltipFunction, options) {
    this.svgGraph.selectAll(".sg-point").call(this._constructTooltipFunctionality(tooltipFunction, options));
    return this;
  };
  /**
  ...
  SimpleGraph.prototype.addTooltipToLines = function (tooltipFunction, options) {
    this.svgGraph.selectAll(".sg-line, .sg-plotted-line").call(this._constructTooltipFunctionality(tooltipFunction, options));
    return this;
  };
  /**
  ...
  SimpleGraph.prototype.addTooltipToAreas = function (tooltipFunction, options) {
    this.svgGraph.selectAll(".sg-plotted-area").call(this._constructTooltipFunctionality(tooltipFunction, options));
    return this;
  };
  /**
...
sg_draw(simple_graph_SimpleGraph, external_amd_d3_root_d3_commonjs_d3_commonjs2_d3_); // Tooltip functions
...
simple_graph_SimpleGraph.prototype.highlightPoints = function (series, validationCallback, size, fill, stylesDict) {
...
simple_graph_SimpleGraph.prototype.removeHighlightPoints = function () {
  this.svgGraph.selectAll(".sg-point-highlight").remove();
  return this;
};

simple_graph_SimpleGraph.prototype.removeHighlights = function () {
