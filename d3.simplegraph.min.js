    ...
    if (this.areas && !~exclude.indexOf("areas")) {
      ...
      for (var _i = 0; _i < this.areas.length; _i++) {
        ...
        if (!~areaSeries.indexOf(name)) {
          ...
          var color = this.getColorBySeriesName(this.areas[_i].series);
          addAreaItem(this.areas[_i], typeof color === "function" ? color(this.areas[_i]) : color);
        }
      }
    } // then lines
    ...
    if (this.lines && !~exclude.indexOf("lines")) {
      ...
      for (var _i2 = 0; _i2 < this.lines.length; _i2++) {
        ...
        if (!~lineSeries.indexOf(_name)) {
          ...
          var _color = this.getColorBySeriesName(this.lines[_i2].series);

          addLineItem(this.lines[_i2], typeof _color === "function" ? _color(this.lines[_i2]) : _color);
        }
      }
    } // finally points
    ...
    if (this.points && !~exclude.indexOf("points")) {
      ...
      for (var _i3 = 0; _i3 < this.points.length; _i3++) {
        ...
        if (!~pointSeries.indexOf(_name2)) {
          ...
          var _color2 = this.getColorBySeriesName(this.points[_i3].series);

          _color2 = typeof _color2 === "function" ? _color2(this.points[_i3]) : _color2;
          addPointItem(this.points[_i3], this.getPointSeriesShape(_name2), _color2, drawPointLine);
        }
      }
...
function sg_data_line_iterableToArray(iter) { if (typeof Symbol !== "undefined" && iter[Symbol.iterator] != null || iter["@@iterator"] != null) return Array.from(iter); }
...
function sg_draw_points_iterableToArray(iter) { if (typeof Symbol !== "undefined" && iter[Symbol.iterator] != null || iter["@@iterator"] != null) return Array.from(iter); }
...
/* harmony default export */ var sg_draw_areas = (function (SimpleGraph, d3) {
  ...
  SimpleGraph.prototype.drawAreas = function (resolution) {
    ...
    this.removeAreas(); // default and enforced minimum resolution for resolving from function
    ...
    this.areas.forEach(function (area) {
      if (area.areaFunctions) {
        area._parts = _this._getAreasPolysFromFunctions(area.areaFunctions[0], area.areaFunctions[1], area.resolution, area.xRange, area.y2, !_this.allowDrawBeyondGraph);
  ...
  SimpleGraph.prototype._drawAreas = function (resolution, transition) {
    ...
    var self = this,
      ...
      return d.series;
    }).attr("class", "sg-area").attr("opacity", transition ? 0 : 1).attr("d", function (d) {
    ...
    if (transition) {
      addedAreas.transition().duration(transition.duration).ease(transition.ease).attr("opacity", 1.0);
    }
  };
  ...
  SimpleGraph.prototype._updateAreas = function (transition) {
    ...
    if (!this.areas) return; // remove, while also filter for new areas
...
simple_graph_SimpleGraph.prototype.clearAllData = function () {
...
simple_graph_SimpleGraph.prototype.getPointsDataBySeries = function (seriesName) {
  var pointData = [];
...
simple_graph_SimpleGraph.prototype.getPointCoordinatesBySeries = function (seriesName) {
  var coordList = [];

  if (this.points) {
    for (var i = 0; i < this.points.length; i++) {
      if (this.points[i].series === seriesName) {
        coordList.push([this.points[i].x, this.points[i].y || this.points[i].y === 0 ? this.points[i].y : this.points[i].y2]);
      }
    }
  }
...
simple_graph_SimpleGraph.prototype.getLinesDataBySeries = function (seriesName) {
  var lineData = [];
...
simple_graph_SimpleGraph.prototype.getAreasDataBySeries = function (seriesName) {
  var areaData = [];
...
simple_graph_SimpleGraph.prototype.drawPoints = function () {
  this.removePoints();
  ...
  var y2Scale = this.y2 ? this.y2.scale : null; // if necessary, remove points that extend beyond graph
  ...
  if (this.allowDrawBeyondGraph) {
    ...
    for (var i = 0; i < this.points.length; i++) {
      ...
      if (this.x["break"]) {
        addPoint = addPoint && (this.points[i].x <= this.x["break"].domain[0] || this.points[i].x >= this.x["break"].domain[0]);
      }

      if (yAxis["break"]) {
        addPoint = addPoint && (this.points[i].y <= yAxis["break"].domain[0] || this.points[i].y >= yAxis["break"].domain[0]);
      }

      if (addPoint) {
        drawPointsData.push(this.points[i]);
      }
    }
  }

  var points = this.svgGraph.selectAll(".sg-point").data(drawPointsData).enter().append("rect").attr("series", function (d) {
    return d.series;
  }).attr("class", "sg-point").attr("width", function (d) {
    return typeof d.pointsize === "function" ? d.pointsize() : d.pointsize;
  }).attr("height", function (d) {
    return typeof d.pointsize === "function" ? d.pointsize() : d.pointsize;
  }).attr("x", function (d) {
...
simple_graph_SimpleGraph.prototype.drawLines = function () {
  this.removeLines(); // for this references

  var self = this;
  var color = this.color;
  var xScale = this.x.scale;
  var svgGraph = this.svgGraph; // local function for adding lines to graph as it may be used multiple times per loop
  ...
  function addLine(lineData, lineCoords, className) {
    ...
    var addedLine = svgGraph.selectAll(".sg-temporary-line").data([lineCoords]).enter().append("path").attr("series", lineData.series).attr("class", className).style("fill", 'none').attr("d", external_amd_d3_root_d3_commonjs_d3_commonjs2_d3_["line"]().x(function (c) {
      return xScale(c[0]);
    }).y(function (c) {
      return yAxis.scale(c[1]);
    }).curve(lineData.interpolate)); // add styles
  ...
  if (this.lines) {
    for (var l = 0; l < this.lines.length; l++) {
      var line = this.lines[l]; // lines added as functions

      if (line.lineFunction) {
        var lineSegments = this._getLineSegmentsFromFunction(line.lineFunction, line.resolution, line.xRange, line.y2Axis, !this.allowDrawBeyondGraph);
...
simple_graph_SimpleGraph.prototype.drawAreas = function () {
  this.removeAreas(); // for this references

  var self = this;
  var color = this.color;
  var xScale = this.x.scale; // local function for adding areas to graph as it may be used multiple times per loop
  ...
  function addArea(areaData, areaCoords, className) {
    ...
    var addedArea = self.svgGraph.selectAll(".sg-temporary-area").data([areaCoords]).enter().append("path").attr("series", areaData.series).attr("class", className).attr("d", external_amd_d3_root_d3_commonjs_d3_commonjs2_d3_["area"]().x(function (c) {
      return xScale(c[0]);
    }).y0(function (c) {
      return yScale(c[1]);
    }).y1(function (c) {
...
simple_graph_SimpleGraph.prototype._findIntercept = function (f, x1, x2, y2Axis) {
  var y1 = f(x1),
      y2 = f(x2);
  ...
  var lasty = y1,
      lastx = x1; // store last x,y values
...
simple_graph_SimpleGraph.prototype._getLineSegmentsFromCoordinates = function (lineCoords, y2Axis) {
  var yAxis = y2Axis ? this.y2 : this.y;
  var lineSegments = [];
  var segment = [];
  var lastCoords = null;
...
simple_graph_SimpleGraph.prototype._getLineSegmentsFromFunction = function (lineFunction, resolution, xRange, y2Axis, limitToGraphRange) {
  var yAxis = y2Axis ? this.y2 : this.y;
  ...
  if (!this.x.isLog) {
    // if not log-scale, standard increment (this works for dates too)
    var increment = (xRange[1] - xRange[0]) / (resolution - 1);
    var isDate = this.x.isDate; // standard increment function
...
simple_graph_SimpleGraph.prototype._getAreaPolysFromCoordinates = function (areaCoordinates, y2Axis) {
  var lineA = [];
  var lineB = [];
...
simple_graph_SimpleGraph.prototype._getAreasPolysFromFunctions = function (funcA, funcB, resolution, xRange, y2Axis, limitToGraphRange) {
  var lines = [this._getLineSegmentsFromFunction(funcA, resolution, xRange, y2Axis, limitToGraphRange), this._getLineSegmentsFromFunction(funcB, resolution, xRange, y2Axis, limitToGraphRange)];
  return this._getAreaPolysFromLineCrosswalk(this._getLineSegmentsFromFunction(funcA, resolution, xRange, y2Axis, limitToGraphRange), this._getLineSegmentsFromFunction(funcB, resolution, xRange, y2Axis, limitToGraphRange), y2Axis);
};

simple_graph_SimpleGraph.prototype._getAreaPolysFromLineCrosswalk = function (lineA, lineB, y2Axis) {
  var areas = [];
  var areaCoords = [];
  var li = [0, 0];
  var ci = [0, 0];
...
simple_graph_SimpleGraph.prototype.removePoints = function () {
...
simple_graph_SimpleGraph.prototype.removeLines = function () {
...
simple_graph_SimpleGraph.prototype.removeAreas = function () {
